...
function selectAll(PDO $pdo) {
    ...
    $liste = array();
    try {
        $lrs = $pdo->query("SELECT * FROM produits");
        $liste = $lrs->fetchAll(PDO::FETCH_ASSOC);
 ...
 * @return null
 */
function selectOne(PDO $pdo, $id) {
    /*
     * Renvoie un tableau associatif
     */
    try {
        $sql = 'SELECT * FROM cours.produits WHERE cp = ?';
        $lrs = $pdo->prepare($sql);
        $lrs->bindValue(1, $id);
        $lrs->execute();
 ...
 * @return type
 */
function insert(PDO $pdo, array $tAttributesValues) {
    $liAffectes = 0;
    try {
 ...
 * @return type
 */
function update(PDO $pdo, array $tAttributesValues) {
    $liAffectes = 0;
    try {
